...
const Modal = ({ story, onClose }) => {
  ...
  const fetchVoices = async () => {
    try {
      ...
      const voiceResponse = await fetch("cp.voicedream.space/api/getVoice", {
        method: "GET",
        headers: {
  ...
  const handleSubmitVoices = async () => {
    try {
      ...
      if (selectedVoicesData.length > 0) {
        const response = await fetch("cp.voicedream.space/api/ttsScript", {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const fetchExtraData = async () => {
    try {
      ...
      const response = await fetch("cp.voicedream.space/api/tts", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
